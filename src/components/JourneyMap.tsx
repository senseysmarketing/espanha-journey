...
const JourneyMap = ({ onSelectMilestone }: JourneyMapProps) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (data) {
        ...
        data.forEach((row) => {
          if (row.milestone_name && row.status === "checked") {
            loaded[row.milestone_name] = true;
          }
        });
  ...
  const handleCheck = useCallback((milestoneId: string, itemIdx: number) => {
    ...
    setCheckedItems((prev) => {
      ...
      if (user) {
        if (next[key]) {
          supabase.from("journey_milestones").upsert(
            { user_id: user.id, milestone_name: key, status: "checked" },
            { onConflict: "user_id,milestone_name" }
          ).then();
        } else {
          supabase.from("journey_milestones")
            .delete()
            .eq("user_id", user.id)
            .eq("milestone_name", key)
            .then();
        }
      }
      return next;
  ...
  const getMilestoneStatus = (m: Milestone): string => {
    const totalItems = m.checklist.length;
    const checkedCount = m.checklist.filter((_, idx) => checkedItems[`${m.id}-${idx}`]).length;
    if (checkedCount === totalItems) return "complete";
    if (checkedCount > 0) return "active";
    return m.status;
  };
    ...
    <div className="w-full max-w-2xl mx-auto px-4 py-8 pb-32">
      ...
      <div className="relative">
        ...
        <div className="space-y-4">
          {milestones.map((m, i) => {
            const computedStatus = getMilestoneStatus(m);
            return (
              <motion.div
                key={m.id}
              ...
              >
                ...
                >
                  <div className="relative z-10 flex-shrink-0 w-12 flex justify-center">
                    <motion.div
                      className={`w-4 h-4 rounded-full ${statusColor(computedStatus)} ${statusGlow(computedStatus)}`}
                      animate={computedStatus === "active" ? { scale: [1, 1.3, 1] } : {}}
                      transition={computedStatus === "active" ? { repeat: Infinity, duration: 2 } : {}}
                    />
                  </div>
